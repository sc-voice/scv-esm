...
const SUID_MAP = path.join(SRC_DIR, 'auto', 'suid-map.json');
...
(async function authors() {
  ...
  const dbg = 0;
  try {
    const authorStats = await AuthorsV2.buildAuthorStats(4);
    dbg && console.log(msg, authorStats);
    let langs = await fs.promises.readdir(
      path.join(EBT_DATA_DIR, 'translation'));
    let authorLangs = {};
