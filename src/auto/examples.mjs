const EXAMPLES = {
  "comment": [
    "Auto-generated by examples.js"
  ],
  "authors": [
    {
      "lang": "de",
      "category": "sutta",
      "version": "1",
      "author": "sabbamitta"
    },
    {
      "lang": "en",
      "category": "sutta",
      "version": "1",
      "author": "sujato"
    },
    {
      "lang": "fr",
      "category": "sutta",
      "version": "1",
      "author": "noeismet"
    },
    {
      "lang": "jpn",
      "category": "sutta",
      "version": "1",
      "author": "kaz"
    },
    {
      "lang": "pt",
      "category": "sutta",
      "version": "1",
      "author": "ebt-deepl"
    },
    {
      "lang": "pt",
      "category": "sutta",
      "version": "2",
      "author": "laera-quaresma"
    }
  ],
  "de": [
    "aber meine Dame",
    "aber zum Erwachen entschlossen",
    "abgefallen",
    "abhängige Entstehen tiefgründig ist",
    "abnehmend",
    "Absicht formt",
    "abwägen, bevor du handelst",
    "ach!",
    "acht Befreiungen",
    "acht Dinge.* entwickelt werden",
    "acht Menschen",
    "Achtsamkeit auf den Tod",
    "Achtsamkeit und meine Weisheit",
    "Achtsamkeit und Situationsbewusstsein",
    "Achtsamkeit verankert",
    "achtsam leben, der Situation bewusst",
    "acht weltliche.* Gegebenheiten",
    "ach, wann wird ein Ausweg",
    "alle äußeren Reize",
    "allein zu wandern",
    "alles Leiden",
    "alle tauglichen Eigenschaften",
    "als ‚Kelch‘, als ‚Schale‘, als ‚Krug‘",
    "als unbeständig erkennt",
    "alt werden m[uü]ss",
    "an einem sicheren Ort",
    "an einen festen Pfosten",
    "Antwort auf diese Frage einfiel",
    "Ärger, Hass und Unmut",
    "Arten der Einbildung",
    "Arten der Fortpflanzung",
    "Art von Lehrer",
    "Asket des.* Lotus",
    "asozial",
    "auf das Auge gedroschen",
    "auf die mittlere Art",
    "auf die Vergangenheit und Zukunft",
    "auf einen zornigen Menschen",
    "auf euch selbst aufpasst",
    "aufgegeben werden m[uü]ss",
    "aufgrund bloßen Vertrauens",
    "Auflösen der Bindungen",
    "Auflösung der Taten",
    "Auflösung des Verlangens ist Erlöschen",
    "auf materiellen Besitz",
    "auf Schönheit",
    "Auge der Weisheit",
    "Augenblick versäum",
    "Ausbauen der Versenkung",
    "Ausdruck für Sinnenfreuden",
    "äußere Erkennen und Sehen",
    "äußere.* Dinge",
    "äußere.* Sinnesfelder",
    "äußerlich gefesselt",
    "ausgeh.* und zurückkomm",
    "aus meiner unmittelbaren Einsicht heraus gelehrt",
    "Bangigkeit",
    "Bauer",
    "bedingte.* Erscheinungen",
    "Bedingungen müssen vergehen",
    "Bedingungen sind unbeständig",
    "Begehren, Abneigung, Dummheit",
    "begrenzte Versenkung und Weisheit",
    "beide.* Enden",
    "bei der besten angekommen",
    "beim Einatmen Seligkeit",
    "bei Tag oder Nacht",
    "bekäme wieder Appetit",
    "beklag",
    "bemüht.*, damit",
    "Benommenheit aufgeben",
    "Bergkuh",
    "Beruhigen.* aller Vorgänge",
    "beschließen ihren Weg",
    "Beschützer",
    "besitzt vier Grundlagen",
    "besser.* erlesener",
    "besteht ein Klargewordener",
    "betrachtet die Erscheinungen",
    "beträchtliches Vermögen",
    "Bevorzugen und Zurückweisen",
    "bewacht.* die Tore der Sinne",
    "bewerte",
    "bewusste Lüge",
    "Bezugspunkte",
    "bis mein Geist",
    "bist du, Zerstörer",
    "blau Seerosen",
    "Blitz",
    "Bodensatz",
    "Bogenharfe",
    "bösartige Unholde",
    "brach in Tränen aus",
    "brutal",
    "Cousin des Buddha",
    "Dachsparren",
    "da ist ein Entrinnen",
    "da mag ein vernünftiger Mensch kommen",
    "damit zufrieden",
    "dankbar",
    "daraus jedes Gefäß herstellen",
    "das allein ist die Wahrheit",
    "das Auge brennt",
    "das Auge, die Bilder und die Fessel",
    "das Auge ist mein",
    "das Auge ist unbeständig",
    "das Auge.* wirklich",
    "das Bewusstsein verankert",
    "das Erste gibt",
    "das Gleiche zu tun",
    "das Glück der Versenkung",
    "das halbe geistliche Leben",
    "das Hoheitsgebiet eurer Väter",
    "das innere Erdelement",
    "das ist die Lehre, das ist die Schulung",
    "das ist eine andere Art",
    "das ist friedvoll",
    "das ist zu eurem Wohl",
    "das Leiden, das im Zugrundegehen liegt",
    "das menschliche Leben",
    "das ohne Selbst ist",
    "das Problem",
    "das rechte Maß",
    "das Selbst ist",
    "dass ich nie sehe oder höre",
    "das Stehlen auf",
    "das unübertreffliche Rad des Dhamma",
    "das vollständige Verstehen aller Arten des Ergreifens",
    "das war meine Ordination",
    "das Weltall ist ewig",
    "das Widerwärtige in dem wahrn.*, was nicht widerwärtig",
    "das zukünftige Auge zu genießen",
    "das Zusammentreffen der drei ist Kontakt",
    "davon genährt",
    "den Besinnungstag",
    "den Buddha bezaubern",
    "den Buddha nie persönlich getroffen",
    "den Dhamma in kurzer Form",
    "den Geist nicht ausrichte.*t, sondern nur hält",
    "den Gipfel und den Kern",
    "den Kreislauf des Leidens zu beenden",
    "den sicheren Pfad",
    "der das Aufhören des Leidens sieht",
    "der Eigentümer",
    "der Hass",
    "der keine Freude empfindet",
    "der Lage bewusst",
    "der siebte Reichtum",
    "der Täter tut nichts Schlechtes",
    "der Versenkung förderlich",
    "der Versteher",
    "dich bekümmern",
    "dich um den Verstand bringen",
    "die Absicht anderer",
    "die Anhänglichkeit losgelassen",
    "die ausführliche Bedeutung",
    "die bedingen könnten",
    "die Besinnung auf",
    "die Erlösung des Herzens nicht reif",
    "die Genügsamen",
    "die ich liebe",
    "die im Wasser",
    "die Kugel aus Seifenpulver",
    "dieser Körper hat dieselbe Natur",
    "dieses Lebewesen",
    "diese Spende gebe",
    "diese Versenkung",
    "diese.* vier edlen Wahrheiten",
    "die Sittlichkeit eines Menschen",
    "die Sorgen vieler zerstört",
    "die Tore zur Unsterblichkeit",
    "die Unbeständigkeit aller Bedingungen",
    "die Unbeständigkeit des Gefühls",
    "die Unbeständigkeit von Bildern",
    "die Unbeständigkeit von Vorstellungen",
    "die Weisen geben",
    "die Welt ist brüchig und wird fortgerissen",
    "die Welt streitet mit mir",
    "die Worte des Buddha wieder",
    "Dinge bei sich selbst",
    "Dinge sind eine Hilfe",
    "disziplinarische Angelegenheit beizulegen",
    "dort sah ich einen riesigen Berg",
    "drei Arten der Versenkung",
    "drei Arten des Verlangens",
    "drei Demonstrationen",
    "drei Dinge.* entwickelt werden",
    "drei Gleichnisse",
    "drei Grundlagen",
    "drei Kennzeichen",
    "drei Menschen",
    "drei Monate in Klausur",
    "drei Schulungen",
    "drei Veden",
    "drei Vorgänge",
    "Dummheit",
    "Dunkelheit.* und Licht",
    "Dunklen, die für das Helle bestimmt",
    "Durchbruch",
    "durch die Versenkung geläutert",
    "durch Glück gewonnen",
    "durch Nahrung erhalten",
    "durch Vertrauen befreit",
    "durch Weisheit befreit",
    "eben da nahm ich beim Buddha",
    "eben das ist das Ende des Leidens",
    "edle Kreislauf",
    "edle Meditation",
    "edle rechte Versenkung mit fünf",
    "eigene Insel",
    "ein abgebrühter Mörder",
    "einäugige Schildkröte",
    "ein Ding muss entwickelt werden",
    "eine Antwort auf diese Frage einfiel",
    "eine Bedingung für Alter und Tod",
    "eine erlesene Nacht",
    "eine fleckenlose Seligkeit",
    "eine große Herausforderung",
    "eine große Hilfe",
    "eine Himmelsrichtung mit einem Herzen voller Liebe",
    "ein Embryo.* empfangen",
    "eine Gefahr für",
    "eine Öllampe, die mithilfe von Öl",
    "eine Straße entlangwanderte",
    "einfältige.* Mann",
    "ein hasserfüllter Mensch",
    "Einheit des Geistes",
    "einen einzigen Geschmack",
    "einen gierigen Geist",
    "einen Schilfhalm aus seiner Scheide",
    "einen verletzenden als auch einen freundlichen Willen",
    "einen Versenkungszustand",
    "eine Wahrnehmung aufkommt",
    "einfühlsam und gütig",
    "eingeäschert",
    "eingeschätzt",
    "ein Hindernis einzig für euch",
    "ein Lehrer mit Namen Sunetta",
    "einnicken",
    "ein paar Jungen",
    "ein paar Palisanderblätter",
    "Einswerden",
    "ein Rest",
    "Eintracht geschaffen",
    "ein Untersucher",
    "Elefant in der Wildnis",
    "Element de",
    "Ende der Welt",
    "Ende des Körpers nahen",
    "entsprechend ihrer Taten wiedergeboren",
    "entwickelt die Grundlage übersinnlicher Kraft",
    "erbärmlich",
    "Erde, Wasser, Feuer und Luft",
    "Ergebnis des Leidens",
    "ergreift.* nichts in der Welt",
    "er hat soviel Verdienst geschaffen",
    "erkannt werden",
    "Erkenntnis der sechs Sinnesfelder",
    "Erkenntnis des Entstehens und Vergehens",
    "erkläre mir das Leiden",
    "Erlangen von materiellem Besitz",
    "erlesenste",
    "erlöschen, bevor ich nicht Nonnenschülerinnen",
    "Erlöschen ist Seligkeit",
    "Erlöschen durch Nicht-Ergreifen",
    "Erlösung des Herzens durch Liebe",
    "ernüchtert vom Verlangen",
    "erscheinen Gedanken",
    "Anfangspunkt",
    "es für dich zu Kummer",
    "es gibt nicht nur",
    "es ist einfach dieser edle achtfache Pfad",
    "es ist mein Pech, mein Missgeschick",
    "es ist zu schwer",
    "es kommt eine Zeit",
    "essen g[ei]b",
    "es war der Besinnungstag",
    "etwas nicht Geringes, das ich bereue",
    "Euter einer Kuh",
    "ewige Wahrheit",
    "Fähigkeit des Gleichmuts",
    "Fähigkeit der Weisheit",
    "falsche Wendung",
    "fasste ihn an den Haaren",
    "Feigheit",
    "Feuerholz sammel",
    "Filz",
    "Fingerschnippen",
    "Floß",
    "Flut.* überquer",
    "Form als das Selbst",
    "Fortziehen nicht vergeblich",
    "Frausein",
    "Frau und Mann",
    "Freiheit von Tadel",
    "Freude, die damit einhergehen",
    "Freude empfinden",
    "Freude und Glück",
    "Frieden finden",
    "friedvoller",
    "friedvolle Meditationen",
    "Fröhlichkeit oder Traurigkeit oder Gleichmut",
    "Frucht des Gebens",
    "fühlt.* es abgelöst",
    "fünf Bestimmungsorte",
    "fünf Dinge.* entwickelt werden",
    "fünf Hindernisse",
    "für das Ziel des Friedens",
    "für dich zu Kummer",
    "für mich eine Beschwerde",
    "für uns getan",
    "für Vertrauen",
    "Galaxie",
    "ganz allein von dir begangen",
    "Gedanken.*kreis",
    "gedeihen unter Wasser",
    "gedenken",
    "gegen den Strom",
    "Gegenstand zur Besinnung",
    "Gegenstände zur Besinnung",
    "gegründete[nr] Gleichmut",
    "geheimen und öffentlichen",
    "gehört immer noch",
    "geht: ‚Ich gehe.‘",
    "geistgeschaffenen Körper",
    "gelichtet",
    "genährt und unterhalten",
    "genas",
    "genießt.*Arbeit",
    "Genügsamkeit",
    "Genussmensch",
    "gepackt",
    "gepflegt werden sollte, und",
    "gepresster Zunge",
    "gerechter und prinzipientreuer König",
    "Gerstengranne",
    "Gerücht",
    "geschlagen und in Panik",
    "geschlossene Faust eines Lehrers",
    "Gesetzmäßigkeiten klar",
    "Gesetzmäßigkeiten mit Weisheit erforscht",
    "gesichert",
    "gesträubten Haare",
    "Gesundheit ist",
    "Gewinn und Verlust",
    "Gewissen und Besonnenheit",
    "gewürdigt, verehrt und wertgeschätzt",
    "gibt dem Besten",
    "gibt es das Erdelement",
    "gibt in dem Gedanken",
    "gibt und bedenkt",
    "Giftschlange",
    "glaubst du",
    "Gleichmut und Versenkung",
    "Gleichnis von der Säge",
    "Glück, das ohne Tadel ist",
    "Glück der Entsagung",
    "Glück und Schmerz sind ewig",
    "Glück und Schmerz sind zugeteilt",
    "Goldschmied",
    "Gottheit, die in diesem Waldstück hauste",
    "göttliche Meditation",
    "göttliche.* strafe",
    "grausam",
    "grenzenlose Versenkung des Herzens",
    "Gründen Tadel",
    "Grundlage der",
    "Grundlage für die Versenkung",
    "gut befreit",
    "gute Freunde, Genossen und Gefährt",
    "gut genug",
    "gut, Schwester",
    "Haare.* sträub",
    "haarsträubend",
    "haltlos werden",
    "hässlich und entstellt",
    "hat.* aufgehört",
    "hätte einen rechtmäßigen Grund",
    "häufig betrachten",
    "Hausbesitzer.* aus Stadt und Land",
    "Hausbesitzer von Bambustor",
    "Heiligtümer",
    "Heimatdorf verließe und zu einem anderen",
    "heißt es so",
    "Hemmnisse und Hindernisse",
    "Herr de[rs]",
    "Herr.* des Waldes",
    "Herzen, das frei von Begehren",
    "Herzenswunsch eines sittlichen Menschen",
    "hier auf der Stelle kühl",
    "Hindernisse auf.*, die Verunreinigungen des Herzens",
    "Hindernisse sind durch und durch",
    "hinters Licht",
    "Hitze wird erzeugt",
    "höchste Reinheit",
    "hoffentlich kommt es so",
    "Hölle de[rs]",
    "Hölle mit Namen",
    "Höllenlärm",
    "hörte mit",
    "ich bin besser",
    "ich bin in den Strom eingetreten",
    "ich bin verrückt geworden",
    "ich fühle ein angenehmes Gefühl",
    "ich komme von Osten",
    "ich lege mich lieber ein bisschen hin",
    "ich lehre das Nicht-Tun",
    "ich sage, es ist nicht möglich",
    "ihm ihre Milch gegeben",
    "ihre Spitze",
    "ihr solltet den Hausbesitzer",
    "im Wünschen nach materiellem Besitz",
    "in äußeren Dingen",
    "in bester Absicht",
    "inbrünstige.* Bußübung",
    "in den drei Zeiten",
    "in den Sinn, zu lehren",
    "in der dunklen Nacht unter freiem Himmel",
    "in der Lage, die Lehre zu verstehen",
    "in der Lehre lebt",
    "in der Versenkung nicht bewandert",
    "in dicken Tropfen auf einen Berggipfel regnen",
    "in die Pflicht",
    "in einem eingeschränkten Sinn",
    "in einem Gefängnis eingesperrt",
    "in Flammen steht",
    "in ihrem ganzen Umfang",
    "in Kummer versunken",
    "in Leiden und Unbehagen",
    "innere Dinge",
    "inneren Sinnesfelder",
    "in Schulden geraten",
    "in seinem Herzen Liebe",
    "ins Leiden verstrickt",
    "interstellaren Raumes",
    "isolierte.* Buddha",
    "isst, was nicht zu tadeln ist",
    "isst wissentlich Fleisch",
    "ist Augenkontakt beständig",
    "ist es wirklich wahr",
    "ist etwas anderes",
    "jammer.* und klag",
    "jedes Bild, das man mit dem Auge sieht, unerwünscht",
    "jegliche Art von Form",
    "jemand anderen vom Geben abhält",
    "jetzt, alt",
    "Kanal zur Wiedergeburt",
    "Kassapa der Prinz",
    "kategorisch",
    "keine Angst, Gutes zu tun",
    "keine Asche und kein Ruß",
    "keine guten Menschen da",
    "keinen Halt",
    "kennt mich",
    "Kennzeichen",
    "Keuschen und die Unkeuschen",
    "kleine.* Hütte",
    "kleine Maß an Vertrauen und Zuneigung",
    "klug, fähig, verständig und gelehrt",
    "kommt der Löwe, der König der Tiere",
    "kommt zuerst die Versenkung",
    "königliches Gut",
    "Königskloster",
    "könnte nicht mein sein",
    "Knoten",
    "Konzept",
    "Kopfhaare, Körperhaare",
    "Kopf in Flammen",
    "Kopf zu den Füßen des Buddha",
    "Kosmologe",
    "Krabbe",
    "Kraft der Weisheit",
    "Kraft des Vertrauens",
    "Krankheit.* Eiterherd.* Pfeil",
    "Kräuter, Hirse",
    "Kreislauf der Wiedergeburten",
    "Kriechtieren abzuhalten",
    "krittelige[nm] Geist",
    "Kuh.* das Leben",
    "Kuhhirt",
    "kühl geworden, voller Seligkeit",
    "Küstenspäher-Vogel",
    "lachende.* Weisheit",
    "lass.* es gut sein mit den sieben Jahren",
    "lass uns unsere Bündel zurücklassen",
    "Läuterung der Ansicht",
    "leere.* Brahmāpalast",
    "leere.* Hütten",
    "Leiden bringen",
    "Leiden vieler Menschen",
    "lernbegierig",
    "lernt die Hymnen",
    "Licht der Weisheit",
    "Licht wahr",
    "lieber hast als dich selbst",
    "liebevolle.* Herz",
    "lodert Freude auf",
    "losgelöst",
    "Macht der Weisheit",
    "Macht über diese Willensbildungsprozesse",
    "man das Auge ergreift",
    "Mangel an Vertrauen",
    "Manifestation",
    "Maßhalten beim Essen",
    "Maus",
    "Meditation unterstützen",
    "meditier konzentrier",
    "meine Form sei so",
    "mein unübertreffliches vollkommenes Erwachen erklärt",
    "merkmalslos",
    "mich sehne und die ich liebe",
    "Milch und Wasser",
    "mir hinterherzulaufen",
    "mir kommt etwas in den Sinn",
    "mir selbst lieb",
    "mit dem Ohr gefolgt",
    "mit den gespitzten Ohren",
    "mit der Auflösung von Gier",
    "mit der Berührung gefangen",
    "mit einem Stock",
    "mit einem Zauber besungen",
    "mit freundlichen Augen",
    "mit ganzem Herzen",
    "mitgenommen, leidend, schwer krank",
    "mit hängenden Schultern",
    "mithilfe eines Gleichnisses",
    "mit ihrem Geist erfasst",
    "mit Keuschheit als fünfter",
    "mit menschlicher Stärke, Energie und Tatkraft",
    "mittlere.* übungsweg",
    "mit ungezügeltem Sehvermögen",
    "mit zuversichtlichem Herzen Zuflucht",
    "Mögliche als möglich",
    "möglich zu entwickeln",
    "Mönche und Nonnen leiten",
    "mörderisch",
    "muss aufgegeben werden",
    "müssen erzeugt werden",
    "nachdem eine sehr lange Zeitspanne",
    "nachdem er gegeben hatte, bereute er es",
    "nach der Mitte greifst",
    "Nachfolger durch Vertrauen",
    "nackt, wahrt den Anstand nicht",
    "nährt das Entstehen",
    "Nahrung für Unwissenheit",
    "Naturgesetz besteht",
    "Neigung zur Einbildung",
    "neun Dinge.* entwickelt werden",
    "neun Faktoren",
    "nicht anders",
    "nicht auf Glauben",
    "nicht auf mündliche Überlieferung",
    "nicht beschämend",
    "nicht euer",
    "nicht möglich, das Ende der Welt",
    "nicht nur hundert",
    "nichts ist es wert, darauf zu bestehen",
    "nicht zur Last",
    "nicht zwei auf einer Straße",
    "nieder, derb, gewöhnlich, unedel und nutzlos",
    "niemals berühmt werde",
    "niemals.* zufriedengeben",
    "nimm meinen Fehler",
    "noch etwas übrig",
    "noch so lange leben",
    "nur einen Bissen",
    "nur mir soll man Gaben geben",
    "Nymphen",
    "ob bei Tag oder bei Nacht",
    "Ochse, der gerne die Saat frisst",
    "ohne die Muster",
    "ohne Grund oder Ursache",
    "ohne seine Aufwärter zu unterrichten",
    "ohne Verlangen",
    "ohne Wahrnehmung",
    "Opferpfahl",
    "Ordenssatzungen",
    "Pañcasikha -sl de",
    "Pfeil des Verlangens",
    "polierte schwarze Schale",
    "Pulsadern",
    "Puppe",
    "Rad-drehend.* Herrscher",
    "rascher Einsicht",
    "redet mich nicht",
    "Regenwolke",
    "Renner",
    "richtig die sieben Faktoren des Erwachens",
    "richtig eingeprägt",
    "Riese",
    "Roben aus Hanf",
    "Rosenapfel",
    "roten Kapok-Wald",
    "rufe alle Mönche und Nonnen",
    "Sack voll Knochen",
    "sagt die Wahrheit",
    "Sammlung als auch Klarsicht",
    "Sammlung und Klarsicht",
    "Sanfteste unter den Sanften",
    "schädlichen Irrglauben",
    "Schale umdrehen",
    "Schaumklumpen",
    "Schilfbündel",
    "Schlange Skorpion Tausendfüßler",
    "schleichend",
    "Schmuckstück aus Beryll von natürlicher",
    "Schneckenhornbläser",
    "schönen Worten",
    "Schüler.* wollen nicht zuhören",
    "Schweinefleisch, das am Umschlagen",
    "Schweiß.* Angesichts",
    "schwer einatmet, weiß",
    "schwer zu erfassen",
    "schwer zu erfüllen",
    "schwer.* zu erreichen",
    "schwer zu finden",
    "schwer zu sehen",
    "schwieg der Buddha",
    "sechs Dinge.* entwickelt werden",
    "sechs Elemente",
    "sechs Gegenstände zur Besinnung",
    "seelische.* Öde",
    "Sehen der Gesetzmäßigkeiten",
    "sehr lange.* Zeit",
    "Sehvermögen nicht zügelt",
    "sehr wohl, Herr",
    "sei.* abhängig entstanden",
    "seinen eigenen geistlichen Gefährten",
    "selbst glücklich und froh",
    "selbst göttliche oder menschliche",
    "Selbstüberwindung",
    "selbst und setz.* andere herab",
    "selbst versteh",
    "selbst wenn er fortgeschickt wird",
    "sich eine Zuflucht bereiten",
    "sicher jemand anders",
    "sicher nicht",
    "sieben Dinge.* entwickelt werden",
    "sieben Gelübde",
    "sieben Tage mit gekreuzten Beinen",
    "sind die Lebewesen von ih[mr] angezogen",
    "Sittlichkeit und.* Weisheit",
    "so hat der Buddha es erklärt",
    "So ist Form",
    "solange sie leben",
    "solche Form in der Vergangenheit",
    "solche Macht und Stärke",
    "soll nach meinem Ende euer Lehrer sein",
    "Sonnenschirm-Orchideenbaum",
    "so viele Eimer",
    "sowohl öffentlich als insgeheim",
    "Spaltung des Saṅgha herbei",
    "Speisen genießen, die aus Vertrauen",
    "Spenden haben keinen Sinn",
    "Spiegelbild",
    "sprich, gutes Horn",
    "Spuren seiner Finger",
    "Staub, der gegen den Wind",
    "steckt nicht in der Mitte",
    "stets wirkt und nie versagt",
    "Stock und Schwert",
    "Straße durch die Wüste",
    "streitet man",
    "Strudel",
    "stufenweise Schulung",
    "substanzialistische Ansicht",
    "Suche nach Befriedigung",
    "süße Spitze",
    "tatsächlich gibt",
    "tauglicher sittlichkeit",
    "Teich mit blauen Seerosen",
    "Textanalyse",
    "tödlichem Schmerz",
    "Tor zur Unsterblichkeit",
    "töte ich",
    "Töten du gutheißt",
    "Träger der Last",
    "Traurigkeit, die mit Gier, Hass und Täuschung",
    "Traurigkeit, die mit Sinnlichkeit",
    "trösten",
    "tut ein Hausbesitzer genug",
    "über das Leiden hinaus",
    "über die Lehre sprechen oder edles Schweigen bewahren",
    "über die Liebe",
    "überhaupt kein Verlangen",
    "überragende Begeisterung",
    "überquer",
    "übersinnliche Kraft und Macht",
    "Übertretung",
    "um die Wesen zu läutern",
    "Umfang der Lehre",
    "Umfang der Weisheit",
    "um mehr materiellem Besitz",
    "unabhängiger Mensch, der gehen könnte",
    "unangebracht, zu einem",
    "unbarmherzig",
    "unbedingt",
    "unbeschwert sein",
    "Unbeständiges als beständig",
    "Unbeständigkeit von Bedingungen",
    "und wenn ich daran sterben würde",
    "unfehlbar",
    "ungerichtet",
    "unmittelbar erkannt werden",
    "unterrichtet angeleitet",
    "unterweise mich über das Leiden",
    "unübertreffliche Dinge",
    "unverdientes Leid zufügt",
    "Unwissenheit schwindet",
    "unwürdige Gespräche",
    "üppige[rn] schlafstelle",
    "Ursprung der Welt",
    "Urteile",
    "Urteil über Menschen",
    "Verantwortung",
    "verbindliches, kategorisches Löwengebrüll",
    "verdammt",
    "veredelt und nährt den Geist",
    "verflucht sei",
    "Vergehen der Erscheinungen",
    "Vergnügen an Willensbildungsprozessen",
    "Verlangen aufgegeben",
    "Verlangen nach Berührungen",
    "Verlangen wird.* aufgegeben",
    "verlasse.* darauf",
    "verloren, wenn er stirbt",
    "Vermögen und Besitz anderer",
    "vernünftiger erwachsener",
    "verpasste Gelegenheit",
    "Verschwinden der wahren Lehre",
    "Versenkung von Sittlichkeit durchdrungen",
    "Versenkung ist friedvoll",
    "Versenkung besitzt",
    "Versenkung des Herzens solcher Art",
    "Versenkung mit fünf Faktoren",
    "Versenkung weiterzuentwickeln",
    "verstecktem Hass",
    "Vertrauen als Partner",
    "Vertrauen und.* Liebe",
    "vertrauensvollen Hausbesitzer",
    "Vervollkommnung de[sr]",
    "verwirklicht werden",
    "viel Leiden und Bedrängnis",
    "vier.* Arten der Suche",
    "vier Arten von Rückhalt",
    "vier Dinge.* entwickelt werden",
    "vier Dinge nicht verstanden und nicht durchdrungen",
    "vierfachen Beschränkung",
    "voller Freude über die Lehre",
    "voller Hochgefühl",
    "vollständig verstanden werden",
    "von all diesen Dingen befreit",
    "von allem Leiden",
    "von den Fußsohlen aufwärts",
    "von der Einbildung geschlagen",
    "von der Kuh stammt die Milch",
    "von der Lehre ableite",
    "von Geburt an blind",
    "von Groll los",
    "von keiner Seite etwas zu befürchten",
    "von Kopf bis Fuß in ein weißes Tuch",
    "von Quellwasser gespeist",
    "von Sprache",
    "von Zorn überwältigt",
    "vor alten Brahmanen",
    "Vorbote und Vorläufer",
    "voreingenommen",
    "vor langer Zeit getan oder gesagt",
    "Wachstum erwarten",
    "während ich in dieser Meditation verweilte",
    "während sie aufkommen",
    "wahrhaftig erkennen und sehen",
    "wahrhaftig versteht: ‚Das ist das Leiden‘",
    "Wahrnehmung des Abstoßenden",
    "Wahrnehmung von Licht",
    "wann immer ich will",
    "warm gehalten und bebrütet",
    "warmherzige Eigenschaft",
    "warum lebe ich nicht allein",
    "was einen Anfang hat",
    "was ein Verstoß ist",
    "was ist mit diesen Menschen",
    "was ist rechte",
    "was ist Unwissenheit",
    "was man noch nicht gehört hat",
    "was noch nicht erreicht ist",
    "Wasser, das auf dem Feuer erhitzt wurde",
    "Wasser, das mit Färbemittel",
    "Wasser, das vom Wind aufgewühlt ist",
    "Wasser, das von Moos und Wasserpflanzen",
    "Wasser, das trüb, dunkel und schmutzig ist",
    "Weberin",
    "weder sich noch andere zu kasteien",
    "weder zustimmen noch.* ablehnen",
    "Weggabelung",
    "Weg zum anderen Ufer",
    "Weisheit auf dem Schoß",
    "Weisheit viel besser als Reichtum",
    "weit weg von der tollen Menge",
    "welche Fesseln binden Götter",
    "Weltall sich zusammenzieht",
    "wem gehört",
    "wenig gelernt",
    "wenig Sand in den Augen",
    "wenn andere mich, die Lehre oder den Saṅgha",
    "wenn das Vertrauen verschwindet",
    "wenn der Faktor des Erwachens Achtsamkeit",
    "wenn die sechs Sinnesfelder zurückweichen",
    "wenn dieses besteht, ist jenes",
    "wenn die Weisheit verschwindet",
    "wenn er es prüft",
    "wenn ich diese Gabe gebe",
    "wenn ich so übe",
    "wenn Kontakt aufhört",
    "wenn sinnliches Begehren",
    "wenn.* stirbt, fühlt",
    "wenn was besteht, weil man was ergreift",
    "werden von.* erhalten",
    "wer spricht diese Strophe",
    "Wesen, das zum Erwachen entschlossen",
    "Wesen ohne Wahrnehmung",
    "wesentliche.* Bedingungen",
    "wesentliche Bedingung für rechte",
    "wie bewacht ein Mönch",
    "Wiedergeburt im Bereich",
    "Wiedergeburt in der Dimension",
    "Wiedergeburt in der Gemeinschaft",
    "Wiedergeburtsklassen",
    "wie der Mond",
    "wie der Mond, der durch die Wolken bricht",
    "wieder und wieder",
    "wie eine Kuh",
    "wie ein Hund",
    "wie ein Tautropfen",
    "wie ein Vogel",
    "wie entwickelt ein Mönch",
    "wie Garn",
    "wie könnte es möglich sein",
    "wie meditiert ein Mönch",
    "wie ein Affe",
    "wie ein.* Katze",
    "wie Menschen ein Haus betreten oder verlassen",
    "wie wächst ein Kind",
    "wilde[sn] Fohlen",
    "Willensbildungsprozesse als das Selbst",
    "Willensbildungsprozesse sind Leiden",
    "Winde wehen nicht",
    "wird nur das",
    "wird verrückt",
    "wo entsteht dieses Verlangen",
    "Worte eines anderen",
    "würde die Schnecken und Muscheln",
    "wurde vo.* nicht erklärt",
    "Wurzel des Leidens",
    "wo würde er auftreffen",
    "Zänker",
    "zehn allumfassende",
    "zehn Dinge.* entwickelt werden",
    "zeig.* den weg",
    "Zombie",
    "zu den jungen Mönchen sagen",
    "zu den Waffen greifen",
    "zu Erhabenheit",
    "zu erkennen oder zu sehen oder",
    "zu ertragen",
    "zu Gericht",
    "zu hören und sich einzuprägen",
    "zum eigenen Nutzen",
    "zu meinem langanhaltenden Nutzen",
    "zum höheren Geist",
    "zum Rückschritt",
    "zum Töten",
    "zur Einheit des Geistes",
    "Zustände seliger Meditation",
    "zu quälen",
    "zusammengelegten Händen Ehrerbietung",
    "zusammenzuck",
    "zu Spänen",
    "zu fest",
    "zu studieren",
    "zwei Arten von",
    "zwei Arten von Glück",
    "zwei Menschen",
    "zwei Dinge.* entwickelt werden",
    "zweifach",
    "zwei Gefühle",
    "Zweifel aufgekommen ist",
    "zwei Toren"
  ],
  "en": [
    "a beryl gem that was naturally beautiful",
    "a ‘pot’, a ‘bowl’, a ‘jar’",
    "accept my mistake",
    "accomplishment in",
    "a condition for old age and death",
    "acquire faith",
    "act after careful consideration",
    "actually",
    "a deep lake fed by spring water",
    "adornment and requisite",
    "advis or instruct",
    "a few rosewood leaves",
    "after a very long period has passed",
    "after giving he regretted it",
    "again and again",
    "against the stream",
    "a hardened killer",
    "a hateful person",
    "a heart full of love to one direction",
    "a heart rid of covetousness",
    "a householder does enough",
    "a just and principled king",
    "alas",
    "a peril for",
    "address me",
    "all right, sir",
    "all skillful qualities",
    "all suffering",
    "all the suffering that arises",
    "an affliction for me",
    "and if I died from that",
    "an embryo is conceived",
    "an escape beyond",
    "angry at an angry person",
    "an inquirer",
    "an obstacle for you alone",
    "an oil lamp depended on oil",
    "announce my supreme perfect awakening",
    "annoyance, hate, and bitterness",
    "anxiety",
    "any kind of form",
    "any kind of pot that they like",
    "a person’s ethics",
    "a person traveling along the road",
    "a raft",
    "a reed out from its sheath",
    "apex",
    "argue with",
    "as long as they live",
    "assess",
    "a state of immersion",
    "as they arise",
    "astute, competent, clever, learned",
    "a Teacher called Sunetta",
    "awakening factor of mindfulness in them",
    "aware of the situation",
    "bag of bones",
    "bearer of the burden",
    "beaten, terminator",
    "become cool right here",
    "becomes established",
    "be comfortable",
    "be completely understood",
    "be directly known",
    "being full of joy",
    "being intent on awakening",
    "beings are aroused by it",
    "belong.* to the higher mind",
    "best of intentions",
    "better.* finer",
    "blameless happiness inside",
    "blamelessness",
    "blind from birth",
    ".*blissful meditations",
    "blue water lilies",
    "both ends",
    "bow to old brahmins",
    "bowl upside down",
    "breathing in heavily they know",
    "brutal",
    "bundles of reeds",
    "burst out in tears",
    "but ma'am",
    "but intent on awakening",
    "but not themselves",
    "by ear",
    "by human strength, energy, and vigor",
    "by means of a simile",
    "by the middle way",
    "can endure",
    "can expect growth",
    "capable of understanding the teaching",
    "categorically",
    "caught by touch",
    "cause a schism",
    "cause of good deeds",
    "causes another to suffer",
    "causes for happiness",
    "cause you sorrow",
    "cause you suffering",
    "certainly not",
    "challenging",
    "characteristics",
    "charm the Buddha",
    "chase after other material possessions",
    "chaste and the unchaste",
    "check.* reflection",
    "choices are suffering",
    "choices as self",
    "claim to propound",
    "classes of rebirth",
    "closed fist of a teacher",
    "collecting firewood",
    "commemorate",
    "complaining",
    "conceit and arrogance",
    "conceit for all",
    "concept",
    "conclude their path",
    "conditioned phenomena",
    "conditioned by which",
    "conditions are impermanent",
    "Conditions fall apart",
    "conducive to immersion",
    "conduit to rebirth",
    "conscience and prudence",
    "contemplate the phenomena",
    "convergence",
    "correctly memorized",
    "cosmologist",
    "cousin",
    "cowardice",
    "cowherd",
    "cow.*took.*life",
    "crab",
    "craving for touches",
    "craving is given up",
    "creating harmony",
    "creepy",
    "criticized on",
    "cross-legged for seven days",
    "cross.* the flood",
    "crown property",
    "cruel",
    "cultivation of immersion",
    "cycle of rebirth",
    "damn",
    "dark bound for light",
    "darkness.* destroyed",
    "darkness vanished",
    "dark will vanish",
    "dart of craving",
    "deadly suffering",
    "defeated and terrified",
    "definitive, categorical lion’s roar",
    "deity haunting that forest",
    "deliberate lie",
    "dependent origination is deep",
    "desire and love",
    "desiring material possessions",
    "destroyer of the sorrows of many",
    "develop immersion in three ways",
    "developing immersion further",
    "develop noble right immersion",
    "developed the seven awakening factors",
    "develop the basis of psychic power",
    "diminished",
    "disappearance of the true teaching",
    "discernment of principles",
    "disciples don’t want to listen",
    "discuss the teachings or keep noble silence",
    "disease.* boil.* dart",
    "disillusioned with craving",
    "dissolute",
    "divine meditation",
    "divine punishment",
    "does a realized one exist",
    "does immersion come first",
    "does it say this",
    "don't accept it",
    "don't fear good deeds",
    "don't go by oral transmission",
    "door to freedom from death",
    "doubt has come up",
    "do you have faith",
    "done for us",
    "dreadful racket",
    "drowsiness arose",
    "dust thrown upwind",
    "earth, water, fire, and air",
    "eats blameless things",
    "eats meat",
    "eight kinds of people",
    "eight liberations",
    "eight people",
    "eight things should be developed",
    "eight worldly conditions",
    "element of",
    "emancipated individual able to go",
    "emotional barrenness",
    "empty huts",
    "empty mansion of Brahmā",
    "enchanted by a spell",
    "encourage others to do the same",
    "ending of attachments",
    "ending of craving is extinguishment",
    "ending of deeds",
    "end of the body approaching",
    "end of the world",
    "end the cycle of suffering",
    "enjoying the eye",
    "entire spectrum",
    "Equanimity and immersion",
    "equanimity based on",
    "establish in",
    "eternal truth",
    "ethics and wisdom",
    "even if sent away",
    "experience an immersion",
    "experiencing bliss",
    "explain suffering to me",
    "exterior sense fields",
    "externally",
    "external stimuli",
    "external things",
    "extinguishment is bliss",
    "extinguished until I have nun disciples",
    "extinguishment by not grasping",
    "eye contact brings happiness",
    "eye of wisdom",
    "faculty of equanimity",
    "faculty of wisdom",
    "faith and love",
    "faith as partner",
    "faithful householders",
    "fallen from",
    "fancy words",
    "farmer",
    "far from the madding crowd",
    "fault-finding mind",
    "favoring and opposing",
    "favoritism",
    "fear and terror",
    "feel it detached",
    "feels inspired to teach",
    "fervent mortification",
    "fettered externally",
    "fewness of wishes",
    "find no footing",
    "find peace",
    "finest",
    "finger-snap",
    "five destinations",
    "five hindrances",
    "five things should be developed",
    "flood so hard to cross",
    "follower by faith",
    "following me around",
    "food given in faith",
    "forerunner and precursor",
    "for faith",
    "fork in the road",
    "foundation of immersion",
    "fourfold constraint",
    "four foundations",
    "four kinds of people",
    "four supports",
    "four things should be developed",
    "freed by faith",
    "freed by wisdom",
    "freed from all these things",
    "from a cow comes milk",
    "from the foolish",
    "from the soles of the feet",
    "fruit of giving",
    "fuel for ignorance",
    "fuels the arising of",
    "full of pleasure",
    "gain and loss",
    "gained through pleasure",
    "getting material possessions",
    "gains wisdom by wanting",
    "gain unification of mind",
    "galaxy",
    "gather all the mendicants",
    "gave him her milk",
    "giant",
    "gives to the best",
    "give thinking",
    "give up drowsiness",
    "give up stealing",
    "give with consideration",
    "giving beauty",
    "giving food",
    "giving gifts",
    "giving this gift",
    "giving up craving",
    "giving up the five",
    "glorify themselves and put",
    "goes naked, ignoring conventions",
    "going forth will not be wasted",
    "goldsmith",
    "go mad",
    "good enough",
    "good friends, companions, and associates",
    "good, sister",
    "goosebumps",
    "grab them by the hair",
    "gradual training",
    "grasp at anything in the world",
    "grasp at the middle",
    "grasping the eye",
    "grateful and thankful",
    "grueling",
    "guaranteed",
    "guard the sense doors",
    "hair-raising",
    "half the spiritual life",
    "happiness or sadness or equanimity",
    "happy and pleased",
    "hard to comprehend",
    "hard to find",
    "hard to see",
    "harmful misconception",
    "harp",
    "has a beginning has an end",
    "has ceased",
    "has not been declared",
    "hatred",
    "have a lie down",
    "have a reaction",
    "have been someone else",
    "having few wishes",
    "head at the Buddha’s feet",
    "head hair, body hair",
    "head w.* on fire",
    "health is",
    "heart of love",
    "heart’s release by love",
    "heat is generated",
    "he has made so much merit",
    "hell called",
    "hell of",
    "herbs, millet",
    "hindrances are entirely",
    "hindrances, corruptions of the heart",
    "homage with joined palms",
    "honey tip",
    "hopefully that’ll happen",
    "horn blower",
    "householders of Bamboo Gate",
    "householders of Nagaravinda",
    "householders of the city",
    "How could it possibly be",
    "how does a child",
    "how does a mendicant",
    "how does a mendicant develop",
    "how does a mendicant guard",
    "how does a mendicant have",
    "how does a mendicant know",
    "how does a mendicant meditate",
    "how many gallons",
    "hurtful and pleasing choices",
    "I am a stream-enterer",
    "I come from the east",
    "I don't rely on faith",
    "I arrived at the best",
    "I feel a pleasant feeling",
    "I feel inspired",
    "if others criticize",
    "if they abuse",
    "ignorance fades away",
    "I get better",
    "I kill",
    "I'll drive you insane",
    "I’m better",
    "if good people are not present",
    "immersion is imbued with ethics",
    "immersion is peaceful",
    "immersion truly",
    "immersion with five factors",
    "impermanence as permanence",
    "imprisoned in a jail",
    "inappropriate to talk",
    "in a qualified sense",
    "in a secure location",
    "in charge",
    "infer to the past and future",
    "inflict",
    "back to the past",
    "for the future",
    "incinerated",
    "independent Buddha",
    "inference from the teaching",
    "in harmony and unity",
    "in order to purify sentient beings",
    "in public and in private",
    "inspired to answer",
    "in suffering and discomfort",
    "intense enthusiasm",
    "intention shapes",
    "interior sense fields",
    "internal joy",
    "internal things",
    "interstellar space",
    "in the open during the dark",
    "into debt",
    "investigate principles",
    "is eye contact permanent",
    "is it really true",
    "is on fire",
    "is quite different",
    "itch",
    "I teach inaction",
    "it is simply this noble eightfold path",
    "it's my loss, my misfortune",
    "it's too hard",
    "I’ve gone mad",
    "I went for refuge ",
    "I will give gifts",
    "joy and happiness",
    "joy springs up",
    "joy that ethical conduct brings",
    "judgment",
    "just one mouthful",
    "just this is the end of suffering",
    "Kassapa the Prince",
    "keep them warm and incubated",
    "killing you approve",
    "kind of teacher",
    "kinds of reproduction",
    "kinds of low",
    "know for yourselves",
    "knowing choices",
    "knowing.*as impermanent",
    "knowledge of the six sense fields",
    "knows me",
    "lack of faith",
    "land-spotting bird",
    "language",
    "laughing wisdom",
    "law of nature persists",
    "lay people",
    "lead the mendicant",
    "lead to distinction",
    "learn new things",
    "leave their home village and go to another",
    "let alone seven years",
    "let a sensible person come",
    "let me develop immersion",
    "let not two go",
    "let’s abandon our bundles",
    "letting go of attachments",
    "liable to grow old",
    "liable to old age",
    "life as a human",
    "light.* of wisdom",
    "like a ball of thread",
    "like a bird",
    "like a cat",
    "like a cow",
    "like a dog",
    "like a drop of dew",
    "like a monkey",
    "lightning",
    "like the moon",
    "like the moon freed from a cloud",
    "limited immersion and wisdom",
    "limitless immersion of heart",
    "listening in on the Buddha",
    "little dust in their eyes",
    "little hut",
    "little learning",
    "live as long",
    "live by",
    "live for",
    "live mindful and aware",
    "lives by the teaching",
    "looking after yourself",
    "lord of the",
    "lost opportunit",
    "lost when.* die",
    "lotus ascetic",
    "loved one",
    "love in their heart",
    "love myself",
    "low, crude, ordinary, ignoble, and pointless",
    "lowlife",
    "low talk",
    "lump of foam",
    "luxurious bed",
    "make a refuge for themselves",
    "make things worse",
    "marks",
    "marks of his fingers",
    "manifestation",
    "may I feel joy",
    "may I never see or hear",
    "may my form be like this",
    "may others not",
    "meaning in giving",
    "meditation did he not praise",
    "meditate and concentrate",
    "mere faith",
    "middle way of practice",
    "might not be mine",
    "milk and water",
    "mindfulness and situational awareness",
    "mindfulness and wisdom",
    "mindfulness of death",
    "mind-made body",
    "mind of love",
    "mind purified by immersion",
    "mind with greed",
    "mired in suffering",
    "miss your moment",
    "moderation",
    "moderation in eating",
    "modes of conceit",
    "monastic codes",
    "more dear to you than yourself",
    "more peaceful",
    "mountain cow",
    "mountain top",
    "mouse",
    "much suffering and distress",
    "my lasting welfare and happiness",
    "neither approve nor dismiss",
    "neither ashes nor soot",
    "never become famous",
    "never be content",
    "nine factors",
    "nine things should be developed",
    "noble cycle",
    "noble meditation",
    "noble search",
    "no craving at all",
    "no danger in any quarter",
    "nodding off",
    "no first point",
    "no form at all",
    "no little remorse and regret",
    "non-percipient",
    "no reason for anyone",
    "not grasping at anything in the world",
    "nothing is worth insisting on",
    "not just one",
    "not mortifying",
    "not otherwise",
    "not personally seen the buddha",
    "not possible to know",
    "not skilled in immersion",
    "not two go by one road",
    "not understanding and not penetrating four things",
    "not yours",
    "now old",
    "nymphs",
    "observing the impermanence of all",
    "obstacles and hindrances",
    "of the sharp ears",
    "oh, when will an escape be found",
    "on a leash",
    "on beauty",
    "one-eyed turtle",
    "one fine night",
    "one taste",
    "one thing should be developed",
    "one who lacks joy",
    "on love",
    "only be given to me",
    "only when they checked",
    "origin of the world",
    "others' intentions",
    "others will",
    "overcome by anger",
    "own island",
    "ox fond of crops",
    "Pañcasikha -sl en",
    "passing away of phenomena",
    "peaceful meditations",
    "penetration",
    "perceive light",
    "perception arises",
    "perception of light",
    "perception of ugliness",
    "pleasure and pain are allotted",
    "pleasure and pain are eternal",
    "pleasure of renunciation",
    "pleasure seeker",
    "polished black bowl",
    "pools fill up the lakes",
    "pool with blue water lilies",
    "pork on the turn",
    "possible to develop",
    "possible to realize",
    "power of wisdom",
    "power over those choices",
    "practices to benefit",
    "practicing like this",
    "prejudice",
    "preoccupation",
    "prevents another from giving",
    "principles for the settlement",
    "protector",
    "psychic power and might",
    "pull a cow",
    "pull the wool",
    "puppet",
    "purification of view",
    "put at ease",
    "rafter",
    "rebirth in the company",
    "rebirth in the dimension",
    "rebirth in the realm",
    "reborn according to their deeds",
    "red silk-cotton wood",
    "references",
    "regain their appetite",
    "regard form as self",
    "rain cloud",
    "recovered",
    "relish work",
    "remember the householder",
    "repulsive in the unrepulsive",
    "restraint of the eye faculty",
    "result of suffering",
    "retreat for three months",
    "revered, venerated, and esteemed",
    "rid of craving",
    "rid of resentment",
    "robes of sunn hemp",
    "root of suffering",
    "rose-apple",
    "Royal Monastery",
    "ruminates as it hunts",
    "rumor",
    "sacrificial post",
    "sadness arises",
    "sadness connected with sensual pleasures",
    "sadness that greed, hate, and delusion bring",
    "said and done long ago",
    "savage monsters",
    "scrupulous and kind",
    "search of.* gratification",
    "secret and public",
    "secret hate",
    "seeing impermanence",
    "see people entering and leaving a house",
    "see the clams and mussels",
    "seized by",
    "self-effacement",
    "sensible and mature",
    "sensual desire in them",
    "serenity and discernment",
    "several boys",
    "serves as your refuge",
    "seven things should be developed",
    "Shady Orchid Tree",
    "shall be your Teacher after my passing",
    "shines a light",
    "should be given up",
    "should be known",
    "should be produced",
    "should be realized",
    "should often review this",
    "shoulders drooping",
    "show.* the way",
    "shrines",
    "sick, suffering, gravely ill",
    "signless",
    "silly man",
    "simile of the saw",
    "six elements",
    "six things should be developed",
    "six topics for recollection",
    "skillful ethics",
    "slain by conceit",
    "snake scorpion centipede",
    "someone with confident heart goes for refuge",
    "something left over",
    "sorrow and wail",
    "speak, good horn",
    "speak the truth",
    "spectrums of the teaching",
    "speedster",
    "spike of rice",
    "starting arguments",
    "startled",
    "still belongs",
    "stilling of all activities",
    "struck in the eye",
    "stuck in the middle",
    "study",
    "stupidity",
    "substantialist view",
    "substantial wealth",
    "such form in the past",
    "such is form",
    "such power and might",
    "suffering in hell",
    "suffering is dependently originated",
    "suffering of many",
    "support meditation",
    "sustained by",
    "strive so that",
    "sweat of the.* brow",
    "swamped by sorrow",
    "swift insight",
    "take pleasure in choices ",
    "take the wealth",
    "take up arms",
    "taking refuge in oneself",
    "tangle",
    "taught from my direct knowledge",
    "tendency to conceit",
    "ten things should be developed",
    "ten universal dimensions",
    "term for sensual pleasures",
    "textual analysis",
    "that’s how the Buddha declared it",
    "that was my ordination",
    "that which you should cultivate",
    "the ball of bath powder",
    "the Buddha kept silent",
    "the cosmos is eternal",
    "the detailed meaning",
    "the Dhamma in brief",
    "the domain of your fathers",
    "the doors to freedom from death",
    "the dregs",
    "the eye faculty unrestrained",
    "The eye is burning",
    "the eye is impermanent",
    "the eye is mine",
    "the eye itself",
    "the foundation of",
    "the fuel for craving",
    "The happiness of immersion",
    "the heart’s release is not ripe",
    "the impermanence of conditions",
    "the impermanence of feeling",
    "the impermanence of sights",
    "the impermanence of ideas",
    "the interior earth element",
    "the known",
    "the lion, king of beasts, emerges",
    "the meeting of the three is contact",
    "the one who acts does nothing wrong",
    "the outcomes of faith",
    "the owner of",
    "the peak and the pith",
    "the possible as possible",
    "the power of faith",
    "the problem",
    "the recollection of",
    "there comes a time",
    "there I saw a huge mountain",
    "there will come a time",
    "there is the earth element",
    "the rod and the sword",
    "the sabbath",
    "these four noble truths",
    "the self is",
    "the seventh kind of wealth",
    "the suffering inherent in perishing",
    "the supreme wheel of dhamma",
    "the sure path",
    "the sweetest of the sweet",
    "the teaching, the training",
    "the three times",
    "the unattained",
    "the wise give",
    "the world is unstable and swept away",
    "they’re happy with that",
    "thief",
    "things are helpful",
    "things in themselves",
    "this body is also of that same nature",
    "this cosmos contracts",
    "this immersion",
    "this is another way",
    "this is for your welfare",
    "this is peaceful",
    "this is the only truth",
    "this is not appropriate",
    "this sentient being",
    "those in water",
    "those of few wishes",
    "thoughts arise",
    "three characteristics",
    "three cravings",
    "three examples",
    "three foundations",
    "three kinds of demonstration",
    "three kinds of immersion",
    "three kinds of people",
    "three practice categories",
    "three processes",
    "three things should be developed",
    "three trainings",
    "three Vedas",
    "thriving underwater",
    "ties",
    "to a strong post",
    "to endure",
    "to go beyond suffering",
    "to kill",
    "to one’s own spiritual companions",
    "too tight",
    "topic for recollection",
    "topics for recollection",
    "to say to the young monks",
    "to wander alone",
    "tongue pressed",
    "train only for peace",
    "transgression",
    "traveling along a desert road",
    "truly knowing and seeing",
    "truly understands: ‘This is suffering’",
    "trust that",
    "tusker in the wild",
    "two feelings",
    "two fools",
    "two kinds of",
    "two kinds of happiness",
    "two kinds of people",
    "five kinds of people",
    "two things should be developed",
    "twofold",
    "ugly and deformed",
    "ultimate purity",
    "unbound",
    "unburdensome",
    "unconditioned",
    "understands the eye, sights, and the fetter",
    "unfailing",
    "undirected",
    "unification of mind",
    "unsullied bliss inside",
    "unsurpassable things",
    "until my mind is freed",
    "vanishing of craving",
    "very helpful",
    "very long time",
    "viper",
    "vital condition for right",
    "vital conditions",
    "voice of another",
    "walking they know",
    "warding off the touch of flies",
    "warm-hearted quality",
    "was done by you alone",
    "water overgrown with moss",
    "water stirred by the wind",
    "water that was cloudy, murky, and muddy",
    "water that was heated by fire",
    "water that was mixed with dye",
    "way to the far shore",
    "weaver",
    "well freed",
    "whatever sight you see with your eye is unlikable",
    "what fetters bind the gods",
    "what is an offense",
    "what is ignorance",
    "what is not-self",
    "what is right",
    "what kind of mendicant",
    "what's up with these people",
    "wheel-turning monarch",
    "when contact ceases",
    "whenever I want",
    "when faith vanishes",
    "when going out and coming back",
    "when the six sense fields recede",
    "when this exists, that is",
    "when what exists, because of grasping",
    "when wisdom vanishes",
    "where does that craving arise",
    "where would it land",
    "whether by day or by night",
    "while I was in that meditation",
    "whirlpool",
    "who does it belong to",
    "who has immersion truly understands",
    "wholeheartedly",
    "who sees the cessation of suffering",
    "who’s speaking this verse",
    "who understood",
    "why don't I live alone",
    "wild colt",
    "will be merely",
    "winds don't blow",
    "wisdom of arising and passing away",
    "wisdom on their lap",
    "wisdom’s much better than wealth",
    "wish of an ethical person",
    "with a stick",
    "with celibacy as the fifth",
    "without informing his attendants",
    "with the end of greed",
    "with kindly eyes",
    "with their own mind",
    "without cause or reason",
    "without learning",
    "without placing the mind, but just keeping it connected",
    "with suffering as outcome",
    "womanhood",
    "works without fail",
    "wrapped from head to foot",
    "wretched",
    "wrists",
    "wrong immersion is blown away",
    "wrong turn",
    "yet I am not a perfected one",
    "wife and husband",
    "zombie"
  ],
  "fr": [
    "racine de la souffrance"
  ],
  "jpn": [
    "死ぬ時、喪失感",
    "老いる定め",
    "愛情による心の解放",
    "美しさを贈り",
    "食べ物を贈る",
    "心で作られた体",
    "真の教えを下落させ消滅させ",
    "願望が少ない",
    "指をパチンと鳴らす",
    "愛情の心",
    "平静さと洞察力",
    "二人の愚か者",
    "深い瞑想の幸福",
    "カラス貝",
    "ゾンビ",
    "控えめ",
    "保証され",
    "五感の扉を防御",
    "青い花の蓮",
    "稲妻のよう",
    "不機嫌さ、憎しみ、恨み",
    "大変よくしてくれ",
    "私だけに贈られる",
    "他の人が贈り物を贈る事を妨げ",
    "金細工職人",
    "高潔な心を目指し",
    "時が来ます",
    "深い瞑想は安らか"
  ],
  "pt": [
    "coisas são úteis",
    "como é que um bhikkhu",
    "conceito",
    "dardo do anseio",
    "desenvolver estes três",
    "devem? ser abandonad.s?",
    "em suportar",
    "escolhas como o eu",
    "esta é outra forma",
    "este nobre caminho óctuplo",
    "forma como o eu",
    "incondicional",
    "não dirigida",
    "queixa",
    "raiva",
    "sem sinais",
    "serenidade e discernimento",
    "três características",
    "três espectros de prática",
    "três processos",
    "vêm do tolo",
    "visão substancialista"
  ]
}

export default EXAMPLES;